...
import { GeoRutasParadas } from '../../interfaces/geo_rutasParadas';
...
export class GeoRutasParadasService {
  ...
  getRutasParadasList(): Observable<GeoRutasParadas[]> {
    return this.http.get<GeoRutasParadas[]>(this.API_URI)
      .pipe(
        catchError(this.handleError)
      );
  }

  getRutasParada(id: number): Observable<GeoRutasParadas> {
    return this.http.get<GeoRutasParadas>(`${this.API_URI}/${id}`)
      .pipe(
        catchError(this.handleError)
      );
  }

  guardarRutasParada(rutaParadas: GeoRutasParadas): Observable<GeoRutasParadas> {
    return this.http.post<GeoRutasParadas>(this.API_URI, rutaParadas)
      .pipe(
        catchError(this.handleError)
      );
  }
