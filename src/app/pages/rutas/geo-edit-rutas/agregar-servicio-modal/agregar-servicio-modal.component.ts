// RUTA COMPLETA: src/app/pages/rutas/geo-edit-rutas/agregar-servicio-modal/agregar-servicio-modal.component.ts
...
// La ruta correcta para takeUntilDestroyed no incluye '/rxjs/'.
...
export class AgregarServicioModalComponent implements OnInit {
  ...
  // Le decimos explícitamente a TypeScript que este control manejará strings.
  ...
  constructor() {
    this.filterControl.valueChanges
      ...
      .subscribe((value) => {
        // Usamos el operador "nullish coalescing" (??) que es más seguro que (||)
  ...
  async cargarServiciosDisponibles(): Promise<void> {
    ...
    try {
      const todosLosServicios = await lastValueFrom(
        this.detalleService.findServiciosDisponiblesParaRuta()
      );
      const serviciosFiltrados = todosLosServicios.filter(
        (s) => !this.data.serviciosActualesIds.includes(s.idServicioEquipo)
      );
